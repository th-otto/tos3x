...
WORD wm_start()
{
	...
	if ((wmhead = (MEMHDR *) Malloc((LONG) (NUMWIN * sizeof(WINDOW) + sizeof(MEMHDR)))) == NULLPTR)
		return FALSE;
	...
	for (wp = (WINDOW *) (wmhead + 1), i = 0; i < NUMWIN; i++, wp++)
	{
	...
	if ((rmhead = (MEMHDR *) Malloc((LONG) (NUMRECT * sizeof(RLIST) + sizeof(MEMHDR)))) == NULLPTR)
		return FALSE;
	...
	*(GRECT *) & (wp->obj[W_WORK].ob_x) = wp->curr;
	...
	if (handle == topw)
	{
		if ((topw = wp->under) != NIL)
		{
			totop(topw, WMCLOSE);
			wp = srchwp(topw);
			ap_sendmsg(wind_msg, WM_ONTOP, wp->owner->p_pid, topw, 0, 0, 0, 0);
		} else
		{
			botw = NIL;
		}
 ...
 *
 */
WORD wm_get(handle, field, ow, iw)
				...
				|| rect.g_w != rect1.g_w || rect.g_h != rect1.g_h || rect.g_x < gl_rfull.g_x)
			{
				...
				if (rect1.g_x != rect.g_x || rect1.g_y != rect.g_y || rect1.g_w > rect.g_w || rect1.g_h > rect.g_h)
				{
					ap_sendmsg(wind_msg, WM_REDRAW, wp->owner->p_pid,
							   handle, wp->curr.g_x, wp->curr.g_y, wp->curr.g_w, wp->curr.g_h);
				}
	...
	case WF_VSLIDE:
	  ...
	  ws_2:
		...
		if (field == WF_HSLIDE || field == WF_HSLSIZE)
		{
			...
			if (wp->hslsz == -1 || (sz = blen * wp->hslsz / 1000) < gl_wbox)
			{
				sz = gl_wbox;
			}
			if (sz >= (ADJ3DPIX << 1))
...
*/
		...
		{								/* field == WF_VSLIDE || field == WF_VSLSIZE */
			...
			if (wp->vslsz == -1 || (sz = blen * wp->vslsz / 1000) < gl_hbox)
			{
				sz = gl_hbox;
			}
			if (sz >= (ADJ3DPIX << 1))
 ...
 *
 */
WORD wm_calc(type, kind, ix, iy, iw, ih, ox, oy, ow, oh)
...
WINDOW *newwp()
{
	...
	{
		if ((mp = (MEMHDR *) Malloc((LONG) (NUMWIN * sizeof(WINDOW) + sizeof(MEMHDR)))) == NULLPTR)
		{
			return (NULLPTR);
		} else
		{
...
int handle;
{
	...
	{
		bucket = handle % NUMWIN;
		for (wp = hashtbl[bucket]; wp != NULLPTR && wp->handle != handle; wp = wp->wnext)
			;
	}
			...
			/* WMSET: called by wm_set()  */
{
	...
	*twp;
	...
	if ((ntwp = srchwp(handle)) == NULLPTR || (caller == WMSET && !ntwp->status.opened))
		return FALSE;
...
WORD handle;							/* handle of window to be bottmed */
{
	...
	*onnbwp;
	...
	ap_sendmsg(wind_msg, WM_ONTOP, unnbwp->owner->p_pid, unnbwp->handle, 0, 0, 0, 0);
...
*/
			...
			if (wp->vslsz == -1)
				...
				w_obrect(wp, W_VELEV, ADJ3DPIX, ADJ3DPIX, gl_wbox, sz * wp->vslsz / 1000);
		}
	}
	...
	if (hparts)
	{
		...
		if (wp->kind & HSLIDE)
		{
			...
			if (wp->hslsz == -1)
				...
				w_obrect(wp, W_HELEV, ADJ3DPIX, ADJ3DPIX, sz * wp->hslsz / 1000, gl_hbox);
		}
	}
