 ...
 *  02/04/93 cjg - Remove Malloc() and use malloc() intead.
 */
 ...
 *==========================================================================
 */
#define MIN_NODES	5					/* Minimum # of nodes    */
						/* for headers       */
	...
	*saved;
	...
	if (hdptr)							/* We might not have ANY CPX headers */
	{
		...
		while (curptr && curptr->vacant)
		{
			 ...
			 */
			if (curptr->cpxhead.flags.bootinit || curptr->cpxhead.flags.resident || curptr->cpxhead.flags.setonly)
			{

				/* update our counters for set-onlys and resident cpxs */
				if (curptr->cpxhead.flags.setonly)
					num_set++;

				if (curptr->cpxhead.flags.resident && !curptr->cpxhead.flags.setonly)
					num_res++;
				...
				if ((!baseptr && !curptr->cpxhead.flags.resident) ||
					(baseptr == (long *) -1L) || (curptr->cpxhead.flags.setonly))
				{
			...
			if (!curptr->cpxhead.flags.setonly)
			{
				cpxsize = curptr->prghead.tsize + curptr->prghead.dsize + curptr->prghead.bsize + CPX_CHICKEN;
...
void Header_Check(CPXNODE * newptr)
{
	...
	{
		if (curptr->vacant && (curptr->cpxhead.cpx_id == newptr->cpxhead.cpx_id))
		{
			 ...
			 */
			if (curptr->cpxhead.cpx_version >= newptr->cpxhead.cpx_version)
			{
...
void handle_cpx(int obj, int nclicks)
{
	...
	*saved;
	...
	if (curptr)
	{
		...
		{
			Set_Active_Node(curptr);
			CPXinit = ((curptr->cpxhead.flags.resident) ? ((void *) *curptr->baseptr) : ((void *) *baseptr));
			 ...
			 */
			if (curptr->cpxhead.flags.resident && !curptr->SkipRshFix)
 ...
 * GLOBALS:  CPXNODE *hdptr:    head pointer to cpx nodes
 */
void move_to_tail(CPXNODE * newptr)
{
	CPXNODE *curptr;
		...
		do
		{
			...
			if ((newptr != NULL) && (newptr != (CPXNODE *) - 1))
			{
				...
				if (!flag || (flag == 1))
				{
					...
					while (curptr)
					{
						if ((curptr->cpxhead.cpx_id == newptr->cpxhead.cpx_id) && curptr->vacant)
							match = TRUE;
						curptr = curptr->next;
					}
					setup_node(newptr, thedta);
					...
					{
						cpxsize = newptr->prghead.tsize + newptr->prghead.dsize + newptr->prghead.bsize + CPX_CHICKEN;
                ...
                */
					if (!match &&
						(newptr->cpxhead.flags.bootinit ||
						 newptr->cpxhead.flags.setonly || newptr->cpxhead.flags.resident))
					{
...
long *load_bootcpx(char *fname, CPXNODE * newptr, BOOLEAN AllowRes)
{
	...
	if (baseptr && (baseptr != (long *) -1L))
	{
		...
		if (cpxinfo)
		{
			...
			if (AllowRes && newptr->cpxhead.flags.resident && !newptr->cpxhead.flags.setonly)
			{
...
BOOLEAN Disk_Check(CPXNODE * curptr, DTA * thedta)
{
	...
	if (flag != -1)
	{
		if ((curptr->cpxhead.cpx_id == tmpnode.cpxhead.cpx_id)
			&& (curptr->cpxhead.cpx_version == tmpnode.cpxhead.cpx_version))
			found = TRUE;
	}
	...
	if (!found && Fsfirst(dirpath, 0x1) == 0)
	{
		do
		{
			...
			{
				if ((curptr->cpxhead.cpx_id == tmpnode.cpxhead.cpx_id)
					&& (curptr->cpxhead.cpx_version == tmpnode.cpxhead.cpx_version) && !tmpnode.cpxhead.flags.setonly)
				{
					found = TRUE;
