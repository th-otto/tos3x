...
int sbindex;							/* index into the save buffer        */
...
BOOLEAN do_option_button(int obj)
{
	...
	{
	case OKOPTION:
		if (ChkTouchButton(tree, OKOPTION))
		{
	...
	case HOUR:
		obj = Pop_Handle(xtree, obj, hour_string, 2, &hour_state, IBM, 10);
		if (obj != -1)
			redraw_hour(obj);
		break;
	...
	case RELOAD:
		cpx_reload(FALSE);				/* FALSE - Called frm XCONFIG */
		break;
...
void save_defaults(void)
{
	...
	if (XGen_Alert(SAVE_DEFAULTS))
	{
		...
		if (buff)
		{
			...
			if ((fd = (int) Fcreate(&savepath[0], 0)) > 0)
			{
