...
int Pop_Handle(OBJECT * tree, int button, char *items[], int num_items, int *default_item, int font_size, int width);
...
void Xconfig_buttons(MRETS * mrets)
{
	...
	if (IsXconfigActive() && IsXtreeActive())
	{
		...
		{
		case OPT_ABOUT:
			redraw_flag = do_about_button(obj);
			break;

		case OPT_INFO:
			redraw_flag = do_info_button(obj);
			break;

		case OPT_XOPTION:
			redraw_flag = do_option_button(obj);
			break;
...
#else
	obj = Pop_Handle(ad_tree, obj, xconfig_string, num_xoptions, &dft, IBM, XOPT_WIDTH);
#endif
