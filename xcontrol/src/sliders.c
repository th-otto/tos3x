...
void cdecl sl_x(OBJECT * tree, int base, int slider, int value, int num_min, int num_max, void (*foo) ());

void cdecl sl_y(OBJECT * tree, int base, int slider, int value, int num_min, int num_max, void (*foo) ());

void cdecl sl_arrow(OBJECT * tree, int base, int slider, int obj,
					int inc, int min, int max, int *numvar, int direction, void (*foo) ());

void cdecl sl_dragx(OBJECT * tree, int base, int slider, int min, int max, int *numvar, void (*foo) ());

void cdecl sl_dragy(OBJECT * tree, int base, int slider, int min, int max, int *numvar, void (*foo) ());

int Calc_Value(int slidxy, int basexy, int basewh, int obwh, int min, int max);
	...
	} else
	{
		ObX(slider) =
			(int) (((long) (value - num_min) *
					((long) (ObW(base) - ObW(slider)) * 1000L)) / ((long) (num_max - num_min) * 1000L));
	}
	if (foo != NULLFUNC)
		(*foo) ();
}
	...
	} else
	{
		ObY(slider) =
			(int) (((long) (value - num_max) *
					((long) (ObH(base) - ObH(slider)) * 1000L)) / ((long) (num_min - num_max) * 1000L));
	}
...
void cdecl sl_dragx(OBJECT * tree, int base, int slider, int min, int max, int *numvar, void (*foo) ())
{
	...
	while (mk.buttons)
	{
		...
		{
			newvalue = Calc_Value(slidx, baserect.g_x, baserect.g_w, ObW(slider), min, max);
		}
...
void cdecl sl_dragy(OBJECT * tree, int base, int slider, int min, int max, int *numvar, void (*foo) ())
{
	...
	while (mk.buttons)
	{
		...
		{
			newvalue = Calc_Value(slidy, baserect.g_y, baserect.g_h, ObH(slider), max, min);
		}
		if (newvalue != lastvalue)
		{
			lastvalue = newvalue;
...
int Calc_Value(int slidxy, int basexy, int basewh, int obwh, int min, int max)
{
	...
	top = (long) ((long) (max - min) * 1000L * (long) (slidxy - basexy - (obwh / 2)));
...
Hot_Slide(OBJECT * tree, int base, int slider, int obj,
		  int inc, int min, int max, int *numvar, int direction, GRECT * rect, void (*foo) ())
{
	MRETS mk;
	int newvalue,
	 oldvalue;
