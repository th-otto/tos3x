 ...
 *	  06/09/93	cjg	-Get rid of adding 4 to the height of the
 *				 work area.
 *				-Don't use Wind_calc(), use wind_calc()
 * DESCRIPTION:
 *
...
#else
	wind_set_str(w.id, WF_NAME, w.name);
#endif
 ...
 * GLOBAL:  WINFO  w:		window structure
 */
void do_redraw(OBJECT * tree, int object, GRECT * prect)
{
	GRECT rect;
