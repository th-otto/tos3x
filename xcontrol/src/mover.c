 ...
 * 02/04/93 cjg - Replace Malloc() with malloc()
 */
...
FNODE *Hot_Old_New_Update(int index, FNODE * Top_Node, int old_index, int findex, int Top_Obj, FNODE * Slit_Array[]);

void Old_Index_Handler(int old_index, int new_index, int findex,
					   FNODE * Top_Node, FNODE * Base_Node, int Above_Obj, int Below_Obj);
...
void Do_Hot_Scroll(int obj, FNODE * Top_Node, FNODE * base_node,
				   ...
				   int Xbase, int Xslider, int *cur_slit, FNODE * Slit_Array[], void (*foo) ());
...
int Cur_ISlit,
 Old_ISlit;
int Cur_ASlit,
 Old_ASlit;								/* The index INTO the filename linked list
...
BOOLEAN mover_button(int obj)
{
	...
	if (xtree == (OBJECT *) rs_trindex[MOVECPX])
	{
		...
		case A3:
		case A4:
			Do_Hot_Scroll(obj, Ahdptr, Active_Slit[obj - A0],
						  Ihdptr, A0, Atotal, ABASE, ASLIDER, &Cur_ASlit, Active_Slit, Draw_AHot);
			break;
		...
		case I3:
		case I4:
			Do_Hot_Scroll(obj, Ihdptr, InActive_Slit[obj - I0],
						  Ahdptr, I0, Itotal, IBASE, ISLIDER, &Cur_ISlit, InActive_Slit, Draw_IHot);
...
int Rename(int flag)
{
	...
	if (flag != TO_NONE)
	{
		...
		{
#if 0
			error2 = Mover_Alert(Head_Node->fname, Alert1A, Alert1B, ((flag == TO_ACTIVE) ? (".CPX") : (".CPZ")));
			error2 = Mover_Alert(Head_Node->fname, Alert2A, Alert2B, ((flag == TO_ACTIVE) ? (".CPZ") : (".CPX")));
	...
	if (count)
	{
		...
		if (Fsfirst(filename, 0x01) == 0)
		{
			do
			{
				...
				for (i = strlen(fname); i && (fname[i] != '.'); fname[i--] = ' ') ;
...
void blit_slits(BOOLEAN flag, int old_slit, int new_slit)
{
	...
	if ((abs(offset) < MAX_SLITS) && rc_equal(&clip2, &clip))
	{
		...
		if (offset > 0)
		{
			objc_offset(tree, obj + abs(MAX_SLITS - offset), &dummy, &rect.g_y);
...
void Set_Move_Text(int flag)
{
	...
	if (((flag == TO_ACTIVE) && Ihdptr) || ((flag == TO_INACTIVE) && Ahdptr))
	{
		if (flag == TO_ACTIVE)
			  ...
			  int Xbase, int Xslider, int *cur_slit, FNODE * Slit_Array[], void (*foo) ())
{
	...
	int data1,
	 data2,
	 data3;								/* storage for list specific items */
		...
		if (Top_Node && Base_Node)
		{
		    ...
		    *************************************************/
					...
					case 6:
						...
						if (old_index != new_index)
						{
							...
							if (New_Node)
							{
								...
								New_Index_Handler(old_index, new_index, findex,
												  Top_Node, Base_Node, (index - 1) + Top_Obj, (index - 1) + Top_Obj);
							}
						}
						old_index = new_index;
						...
						{
							Hot_Old_New_Update(index, Top_Node, old_index, findex, Top_Obj, Slit_Array);
					...
					{
						Hot_Old_New_Update(hot_index, Top_Node, old_index, findex, Top_Obj, Slit_Array);
					}
...
FNODE * Hot_Old_New_Update(int index, FNODE * Top_Node, int old_index, int findex, int Top_Obj, FNODE * Slit_Array[])
{
	...
	if (New_Node)
	{
		...
		Old_Index_Handler(old_index, new_index, findex, Top_Node, Base_Node, (index - 2) + Top_Obj, index + Top_Obj);
