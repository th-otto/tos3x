...
void fixitup(PBP * pbp, unsigned char *fixbuf)
{
	char *curpos;
	unsigned char fixup;
	long firstfix;
...
long loadcpx(const char *name)
{
	...
	if ((Fread(handle, sizeof(head), &head) != sizeof(head)) || (head.magic != 0x601a))
		goto loaderr;

	pbp = loadbuf;
	if (!pbp)
		pbp = (struct PseudoBP *) malloc(sizeof(pbp) + head.tsize + head.dsize + head.bsize + CPX_CHICKEN);
	if (!pbp)
		goto loaderr;
