...
#if 0
			/* Do this ONLY if AES Version is >= 0x0330 */
			if ((AES_Version >= 0x0320) && (gl_ncolors > LWHITE))
			{
				IconColor(slots[index].icon) = (IconColor(slots[index].icon) & 0xf0ff) | (LWHITE << 8);
				TedColor(slots[index].text) &= 0xFF7F;
			}
#endif
			curptr = curptr->next;
		}
 ...
 * GLOBAL:  SLOTS slots:    slot array structure
 */
void clear_active_slot(int redraw)
{
	OBJECT *tree = (OBJECT *) rs_trindex[XCONTROL];
...
void blit_slots(int oldnum, int newnum)
{
	...
	if ((abs(offset) < MAX_SLOTS) && rc_equal(&clip2, &clip) && (ypos2 < ypos1))
	{
		rect.g_h = (abs(offset) * ObH(SLOT0));
		dstbase.g_h = srcbase.g_h = (MAX_SLOTS - abs(offset)) * ObH(SLOT0) + (MAX_SLOTS - abs(offset));
		if (offset > 0)
		{
			objc_offset(tree, slots[abs(MAX_SLOTS - offset)].obj, &dummy, &rect.g_y);
