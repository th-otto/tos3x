...
CPXINFO *cdecl cpx_init(XCPB * Xcpb)
{
	...
	if (AES_Version < 0x0400)		/* if < version 0400 of AES */
	{
		form_alert(1, alert2);
...
int InitStrings(char *bufptr)
{
	...
	while (length && count < MAX_NODES)
	{
		 ...
		 */
		if (strlen(ESTRING(curptr)) > TEXT_LENGTH)
			ESTRING(curptr)[27] = '\0';

		/* pad the shorter strings to the length of the buffer ( TEXT_LENGTH ) */
		while (strlen(ESTRING(curptr)) < TEXT_LENGTH)
...
void BlitSlots(int old_slit, int new_slit)
{
	...
	if ((abs(offset) < MAX_SLOTS) && rc_equal(&clip2, &clip))
	{
		rect.g_h = (abs(offset) * ObH(obj));
		dstbase.g_h = srcbase.g_h = (MAX_SLOTS - abs(offset)) * ObH(obj);
		if (offset > 0)
		{
			objc_offset(tree, obj + abs(MAX_SLOTS - offset), &dummy, &rect.g_y);
...
void UndoNodes(void)
{
	...
	FreeBuffer();
	initialize();
	Objc_draw(ad_tree, TBASE, MAX_DEPTH, NULL);
	Objc_draw(ad_tree, XBASE, MAX_DEPTH, NULL);
}
...
static void objc_xywh(OBJECT *tree, WORD obj, GRECT *r)
{
	objc_offset(tree, obj, &r->g_x, &r->g_y);
	r->g_w = tree[obj].ob_width;
	r->g_w = tree[obj].ob_height;
}
...
void EnableDelete(void)
{
	...
	if (IsDisabled(XDELETE))
	{
		...
		MakeExit(XDELETE);
		Enable(XDELETE);
		Objc_draw(ad_tree, DUMMY2, MAX_DEPTH, &rect);
	}
}
...
void save_cnf(void)
{
	...
	if (form_alert(1, alert5) == 1)
	{
		...
		{
			/* found it! */
			if ((cnfptr = (char *) Malloc(newdma.d_length * 2L)) != NULL)
			{
				cnfsize = newdma.d_length;
