 ...
 *			etc...
 */
void Write_Port(int port, BUFFER * store)
{
 ...
 */
void Read_Port(int port, BUFFER * store)
{
	int UCR_Data;
 ...
 */
void Set_Rsconf(int port)
{
	Set_BaudRate(Modem.modem[port].cur_baud);
	Set_FlowControl(Modem.modem[port].cur_flow);
	Set_UCR(Modem.modem[port].cur_parity, Modem.modem[port].cur_stop, Modem.modem[port].cur_bits);
}
