...
static void cdecl cpx_draw(GRECT *clip)
{
	GRECT rect;
 ...
 */
static int CheckBox(int boxnum, int iconnum)
{
	int dummy;
	int i;
	GRECT rect;
 ...
 * ==================================================================
 */
static BOOLEAN GetWindow(int item)
{
	GRECT rect;

	if (WHandle < 0)
	{
		if ((WHandle = wind_create(CLOSER | FULLER | MOVER | SIZER | NAME, 0, 16, 640, 480)) < 0)
		{
			WHandle = NIL;
...
static void cdecl cpx_mover(GRECT *work)
{
	ActiveTree(ad_tree);
	ObX(ROOT) = work->g_x;
	ObY(ROOT) = work->g_y;
...
static void do_system(int *msg)
{
	...
	case WM_REDRAW:
		...
		while (rect.g_w && rect.g_h)
		{
			if (rc_intersect(&clip, &rect))
			{
				Objc_draw(ad_xtree, ROOT, MAX_DEPTH, &rect);
			}
...
static void cdecl cpx_close(BOOLEAN flag)
{
	(void)flag;
	if (WHandle > 0)
	{
...
static void cdecl cpx_button(MRETS * mrets, int nclicks, int *event)
{
	int obj;
	int box;
	MRETS mk;
...
static void cdecl cpx_key(int kstate, int key, int *event)
{
	int obj;
	int box;
 ...
 */
static BOOLEAN cdecl cpx_hook(int event, int *msg, MRETS * mrets, int *key, int *nclicks)
{
	int topid;
	int dummy;
	int obj;
