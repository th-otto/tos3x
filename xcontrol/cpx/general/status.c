...
#if 0
void do_memory(int Total_object, char *xtotal, int Free_object, char *xfree, int device);
#endif
 ...
 */
#if 0
void do_memory(int Total_object, char *xtotal, int Free_object, char *xfree, int ram_item)
#endif
void do_memory(int Free_object, char *xfree, int ram_item)
{
	OBJECT *ad_partz = (OBJECT *) rs_trindex[PARTZ];
	OBJECT *tree;
...
long *Fmalloc(long num, int item)
{
	...
	if ((*xcpb->getcookie) (0x5f465242L, &p_value))
	{
		if (item == TTRAM)
		{
			...
			if ((_ramvalid == _MAGIC) && (_ramtop > 0x1000000L))
			{
...
long get_total_ram(void)
{
	...
	Total_Ram = 0L;
	if (((*xcpb->getcookie) (0x5f465242L, &p_value)) && Ram_Device == TTRAM)
	{
		_ramvalid = *(long *) 0x5a8;
		_ramtop = *(long *) 0x5a4;
