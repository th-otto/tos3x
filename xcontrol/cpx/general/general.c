			...
			if ((AES_Version >= 0x0330) && (numcol > LWHITE))
			{
				...
				for (i = RESPBASE; i <= LRESP; i++)
				{
					...
					if ((ObType(i) & 0x00ff) == G_BOXCHAR)
						ObIndex(i) = (ObIndex(i) & 0xffffff70L) | LWHITE | 0x70;
...
int handle_button(int button, WORD * msg)
{
	...
	case BELL:
		keybell_handle(BELLICON, &cur_value.bell);
		break;
	...
	case M3:
	case M4:
		Dclick_Select(button);
		break;

	case DOUBLE:
		Dclick_Handle(click_flag, button);
		break;
	...
	case KREPEAT:
		...
		{
			(*xcpb->Sl_dragx) (tree, REPTBASE, KREPEAT, KEYMIN, KEYMAX, &cur_value.repeat, KRepeat);
			Set_Kbrate();
		}
	...
	case KRESPONS:
		...
		if (mk.buttons && (mk.x != ox))
		{
			(*xcpb->Sl_dragx) (tree, RESPBASE, KRESPONS, KEYMIN, KEYMAX, &cur_value.response, KResponse);
			Set_Kbrate();
		}
	...
	case RRESP:
		(*xcpb->Sl_arrow) (tree, RESPBASE, KRESPONS, RRESP,
						   1, KEYMIN, KEYMAX, &cur_value.response, HORIZONTAL, KResponse);
		Set_Kbrate();
		break;

	case LRESP:
		(*xcpb->Sl_arrow) (tree, RESPBASE, KRESPONS, LRESP,
						   -1, KEYMIN, KEYMAX, &cur_value.response, HORIZONTAL, KResponse);
		Set_Kbrate();
		break;

	case RREPEAT:
		(*xcpb->Sl_arrow) (tree, REPTBASE, KREPEAT, RREPEAT, 1, KEYMIN, KEYMAX, &cur_value.repeat, HORIZONTAL, KRepeat);
		Set_Kbrate();
		break;

	case LREPEAT:
		(*xcpb->Sl_arrow) (tree, REPTBASE, KREPEAT, LREPEAT,
						   -1, KEYMIN, KEYMAX, &cur_value.repeat, HORIZONTAL, KRepeat);
		Set_Kbrate();
		break;
	...
	case REPTBASE:
		...
		oy = ((mk.x < ox) ? (-3) : (3));
		(*xcpb->Sl_arrow) (tree, REPTBASE, KREPEAT, -1, oy, KEYMIN, KEYMAX, &cur_value.repeat, HORIZONTAL, KRepeat);
		Set_Kbrate();
		break;
	...
	case RESPBASE:
		...
		(*xcpb->Sl_arrow) (tree, RESPBASE, KRESPONS, -1,
						   oy, KEYMIN, KEYMAX, &cur_value.response, HORIZONTAL, KResponse);
		Set_Kbrate();
		break;
