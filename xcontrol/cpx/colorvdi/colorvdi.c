...
#ifdef __TURBOC__
#define WindGet(handle, kind, r) wind_get(handle, kind, &(r)->g_x, &(r)->g_y, &(r)->g_w, &(r)->g_h)
void ObjcDraw(OBJECT *tree, WORD obj, WORD depth, GRECT *r)
{
	Objc_draw(tree, obj, depth, r);
}
#define EvntButton(clicks, mask, state, mk) evnt_button(clicks, mask, state, &(mk)->x, &(mk)->y, &(mk)->buttons, &(mk)->kstate)
#endif
...
void update_slid(int base, int slider, int value, int min, int max, int draw);
void slidtext(void);
...
BOOLEAN cdecl cpx_call(GRECT *rect)
{
	...
	do
	{
		...
		case CUP:
			(*xcpb->Sl_arrow) (tree, CBASE, CSLIDE, CUP, -1, col_max, col_min, &curcol, VERTICAL, nxtrow);
			break;

		case CDOWN:
			(*xcpb->Sl_arrow) (tree, CBASE, CSLIDE, CDOWN, 1, col_max, col_min, &curcol, VERTICAL, nxtrow);
			break;
		...
		case CSLIDE:
			...
			} else
			{
				...
				(*xcpb->Sl_dragy) (tree, CBASE, CSLIDE, col_max, col_min, &curcol, nxtgrp);
		...
		case CBASE:
			...
			if (mk.y < oy)
				...
				oy = col_page;
			curslid = CSLIDE;
			(*xcpb->Sl_arrow) (tree, CBASE, CSLIDE, -1, oy, col_max, col_min, &curcol, VERTICAL, nxtpage);

			break;


		case RUP:
			Do_Up(RBASE, RSLIDE, RUP, R);
			break;

		case RDOWN:
			Do_Down(RBASE, RSLIDE, RDOWN, R);
			break;

		case RSLIDE:
			Do_Slider(RBASE, RSLIDE, R, dclick);
			break;

		case RBASE:
			Do_Base(RBASE, RSLIDE, R);
			break;

		case GUP:
			Do_Up(GBASE, GSLIDE, GUP, G);
			break;

		case GDOWN:
			Do_Down(GBASE, GSLIDE, GDOWN, G);
			break;

		case GSLIDE:
			Do_Slider(GBASE, GSLIDE, G, dclick);
			break;

		case GBASE:
			Do_Base(GBASE, GSLIDE, G);
			break;


		case BUP:
			Do_Up(BBASE, BSLIDE, BUP, B);
			break;

		case BDOWN:
			Do_Down(BBASE, BSLIDE, BDOWN, B);
			break;

		case BSLIDE:
			Do_Slider(BBASE, BSLIDE, B, dclick);
			break;

		case BBASE:
			Do_Base(BBASE, BSLIDE, B);
			break;
...
void update_slid(int base, int slider, int value, int min, int max, int draw)
{
	...
	if (draw)
	{									/* if requested to draw, draw the slider */
...
void update_rgb(int draw)
{
	...
	if (oldscrn[R] != curscrn[R])
		update_slid(RBASE, RSLIDE, curscrn[R], 0, 1000, draw);

	if (oldscrn[G] != curscrn[G])
		update_slid(GBASE, GSLIDE, curscrn[G], 0, 1000, draw);

	if (oldscrn[B] != curscrn[B])
		update_slid(BBASE, BSLIDE, curscrn[B], 0, 1000, draw);
}
...
void nxt_to_show(int toscroll)
{
	...
	} else if (curcol >= headcol + MAX_COL_SHOWN)
	{									/* page forward */
...
{
	curslid = slider;
	(*xcpb->Sl_arrow) (tree, base, slider, button, 1, 0, 1000, &curscrn[index], VERTICAL, adjcol);
}
...
void Do_Slider(int base, int slider, int index, int dclick)
{
	...
	else
	{
		...
		if (mk.buttons && (mk.y != oldy))
			(*xcpb->Sl_dragy) (tree, base, slider, 0, 1000, &curscrn[index], adjcol);
...
void Do_Base(int base, int slider, int index)
{
	...
	if (mk.y < oy)
		...
		oy = -VAL_PAGE;
	curslid = slider;
	(*xcpb->Sl_arrow) (tree, base, slider, -1, oy, 0, 1000, &curscrn[index], VERTICAL, adjcol);
}
...
void Do3D(void)
{
	...
	ActiveTree(ad_tree);
	ObIndex(BASER) = ObIndex(BASER) & 0xffff0fffL;
	ObX(BASER) += 1;
	ObY(BASER) += 1;
	ObW(BASER) -= 2;
	ObH(BASER) -= 2;
