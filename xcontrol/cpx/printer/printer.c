	...
	} else
	{
		...
		if (!xcpb->SkipRshFix)
			(*xcpb->rsh_fix) (NUM_OBS, NUM_FRSTR, NUM_FRIMG, NUM_TREE,
							  ...
							  rs_iconblk, rs_bitblk, rs_frstr, rs_frimg, rs_trindex, rs_imdope);
...
BOOLEAN cdecl cpx_call(GRECT * rect)
{
	...
	do
	{
...
#if GERMAN
		case PTYPE:
			Pop_Handle(PTYPE, type_array, 2, &cur_value.cur_type, IBM, 11);
			break;
#endif

#if FRENCH | SPAIN | SWEDEN | ITALY
		case PTYPE:
			Pop_Handle(PTYPE, type_array, 2, &cur_value.cur_type, IBM, 10);
			break;
#endif
...
#if USA | UK
		case PTYPE:
			Pop_Handle(PTYPE, type_array, 2, &cur_value.cur_type, IBM, 8);
			break;
#endif
...
#if FRENCH
		case COLOR:
			Pop_Handle(COLOR, color_array, 2, &cur_value.cur_color, IBM, 10);
			break;
#endif

#if ITALY
		case COLOR:
			Pop_Handle(COLOR, color_array, 2, &cur_value.cur_color, IBM, 9);
			break;
#endif

#if USA | UK | SPAIN | GERMAN | SWEDEN
		case COLOR:
			Pop_Handle(COLOR, color_array, 2, &cur_value.cur_color, IBM, 8);
			break;
#endif
		...
		case PIXLINE:
			Pop_Handle(PIXLINE, pix_array, 2, &cur_value.cur_pixel, IBM, 8);
			break;
		...
		case QUALITY:
			Pop_Handle(QUALITY, quality_array, 2, &cur_value.cur_quality, IBM, 10);
			break;
#endif

#if SWEDEN
		case QUALITY:
			Pop_Handle(QUALITY, quality_array, 2, &cur_value.cur_quality, IBM, 9);
			break;
#endif

#if USA | UK | SPAIN
		case QUALITY:
			Pop_Handle(QUALITY, quality_array, 2, &cur_value.cur_quality, IBM, 8);
			break;
#endif
...
#if ITALY
		case PAPER:
			Pop_Handle(PAPER, paper_array, 2, &cur_value.cur_paper, IBM, 10);
			break;
#endif

#if USA | UK | SPAIN | GERMAN | FRENCH
		case PAPER:
			Pop_Handle(PAPER, paper_array, 2, &cur_value.cur_paper, IBM, 9);
			break;
#endif

#if SWEDEN
		case PAPER:
			Pop_Handle(PAPER, paper_array, 2, &cur_value.cur_paper, IBM, 8);
			break;
#endif
...
#if ITALY
		case PORT:
			Pop_Handle(PORT, port_array, 2, &cur_value.cur_port, IBM, 9);
			break;
#else
		case PORT:
			Pop_Handle(PORT, port_array, 2, &cur_value.cur_port, IBM, 10);
			break;
#endif
		default:
			if (button == -1)
...
void Set_Printer(PRT * value)
{
	...
	current |= (value->cur_type |
				(value->cur_color << 1) |
				(value->cur_pixel << 2) | (value->cur_quality << 3) | (value->cur_port << 4) | (value->cur_paper << 5));
	Setprt(current);
}
