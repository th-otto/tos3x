 ...
 * 04/28/92 cjg - supports AP_TERM now - treats it like an AC_CLOSE
 * 06/11/93 cjg - Use a global rectangle GRECT to return the FIRST and
 *		  NEXT GRECT.
 * 06/21/93 cjg - Fixed up wind_update calls to work with TOS 1.2
 */
 ...
 *==========================================================================
 */
#define AP_TERM		50					/* Termination message */
#ifndef CT_KEY
#define CT_KEY 53
#endif
...
BOOLEAN cursor = FALSE;					/* TRUE - Cursor is ON!
										 ...
										 */
GRECT gl_rect;							/* Global rect for
										 * FIRST_XYWH and NEXTXYWH
										 */
 ...
 * `direction'.
 */
static WORD find_obj(OBJECT * tree, WORD startob, DIRS direction)
{
	WORD obj,
	 searchflag,
	 curflag,
 ...
 */
BOOLEAN fm_button(OBJECT * tree, WORD obj, WORD clicks, WORD * pobj)
{
	WORD state,
	 flags,
	 parent,
	...
	if ((flags & SELECTABLE) && !(state & DISABLED))
	{
		...
		if (flags & RBUTTON)
		{
			...
			for (tobj = ObHead(parent); tobj != parent; tobj = ObNext(tobj))
			{

				state = ObState(tobj);
				if ((ObFlags(tobj) & RBUTTON) && ((state & SELECTED) || (tobj == obj)))
				{
					if (tobj == obj)
						state |= SELECTED;
					else
 ...
 */
BOOLEAN can_edit(OBJECT * tree, WORD obj)
{
	GRECT r;
	WORD xy[4];
	int top_window;
...
#else
	wind_get_int(w.id, WF_TOP, &top_window);	/* cjg */
#endif
	if (w.id == top_window)
	{
		r = ObRect(obj);
		...
		rc_2xy(&r, xy);
		return (xy_inrect(xy[0], xy[1], &w.work) && xy_inrect(xy[2], xy[3], &w.work));
	} else
		return (FALSE);
}
...
WORD cdecl xform_do(OBJECT * tree, WORD start_field, WORD puntmsg[])
{
	...
	while (cont)
	{
		 ...
		 * Wait...
		 */
		event = Evnt_multi(MU_KEYBD | MU_BUTTON | MU_MESAG, 2, 1, 1, NULL, NULL, msg, 0L, &m, &key, &clicks);
 ...
 */
		if (EvKey())
		{
			...
			if (direction != NODIR)
			{
				...
				{
					Objc_change(tree, next_obj, &w.work, ObState(next_obj) | SELECTED, TRUE);
					cont = FALSE;
				}
			}
			/*
			...
			if (key && (!(key & 0xff)))
			{
				if ((key != K_UP) &&
					(key != K_DOWN) &&
					(key != K_RIGHT) &&
					(key != K_LEFT) && (key != K_RETURN) && (key != K_ENTER) && (key != K_TAB) && (key != K_BACKTAB))
				{
					puntmsg[0] = CT_KEY;
					puntmsg[3] = key;
