 ...
 */
WORD ChkTouchButton(tree, button)
OBJECT *tree;
	...
	do
	{
		...
		case SVOK:
			if (((cstart != old_cstart) ||
				 (lstart != old_lstart) || (dstart != old_dstart) || (old_mode_code & STMODES)) && monitor_type)
			{
				st_flag = FALSE;
				mode_code = ((cstart - VID2) |
							 ((lstart == SVL80) ? (COL80) : (COL40)) |
							 vga_flag | pal_flag | overscan_flag | st_flag | ((dstart == SVION) ? (VERTFLAG) : (0)));
 ...
 */
WORD DoPopup(tree, button, title, Mtree, Mmenu, Mfirst, Mstart, Mscroll, FirstMenu, FirstText, Skip)
OBJECT *tree;
 ...
 */
WORD DoMKItems(dtree, button, item)
OBJECT *dtree;
