 ...
 */
static VOID tadjust(P(struct tnode *) tp, P(int) type, P(int) dp, P(int) ssp)
PP(register struct tnode *tp;)						/* pointer to tree */
 ...
 */
static int specops(P(int) op, P(struct tnode *) ltp, P(struct tnode *) rtp)
PP(int op;)									/* operator */
PP(register struct tnode *ltp;)						/* left subtree pointer */
PP(register struct tnode *rtp;)						/* right subtree pointer */
{
	register short type;
...
#ifdef DEBUG
	if (treedebug)
		fprintf(stderr, "specops: op = %d type = %d\n", op, type);
#endif
	switch (op)
	{
	case 0:
...
PP(int op;)									/* for cast operator */
{
	...
	if (treedebug)
	{
		fprintf(stderr, "cvopgen: type=%d conv=%d op=%d ", type, conv, op);
		fprintf(stderr, "- type=%d op=%d\n", tp->t_type, tp->t_op);
	}
#endif
 ...
 */
static struct tnode *funcref(P(struct tnode *) tp)
PP(register struct tnode *tp;)						/* pointer to old node */
{
 ...
 */
static struct tnode *arrayref(P(struct tnode *) tp)
PP(register struct tnode *tp;)
{
	register short dp;
...
static int ttoconv(P(int) type)					/* returns conversion type */
...
PP(int op;)									/* new root operator */
{
	...
	} else
	{
		lconv = ttoconv(ltype);
		conv = ttoconv(rtype);
		if (ISASGOP(op) || lconv >= conv ||
			(COMOP(op) && ltype == INT && rtype == LONG && rtp->t_op == CLONG))
		{
