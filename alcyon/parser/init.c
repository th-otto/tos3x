 ...
 */
static int out_bfield(P(int) type)
PP(int type;)
{
	in_bitinit = 0;
	if (bfield_ty == CHAR)
 ...
 */
static int oneinit(P(int) type, P(int) dp, P(int) sc)
...
#ifdef DEBUG
			if (initdebug)
				fprintf(stderr, "s_or_a: child %s type %d\n", child->s_symbol, onetype);
#endif
		}
...
#ifdef DEBUG
			if (initdebug)
				fprintf(stderr, "s_or_a recurse onetype STRUCT\n");
#endif
			snest = (next(LCURBR));
			temp = nbout;
...
#ifdef DEBUG
				if (initdebug)
					fprintf(stderr, "BASIC_AR onetype 0%o\n", onetype);
#endif
...
#ifdef DEBUG
				if (initdebug)
					fprintf(stderr, "BASIC_AR original 0%o\n", original);
#endif
...
#ifdef DEBUG
				if (initdebug)
					fprintf(stderr, "ISARRAY onetype 0%o\n", onetype);
#endif
...
#ifdef DEBUG
		if (initdebug && child)
			fprintf(stderr, "child [%s] PEEKTOK comma %d\n", child->s_symbol, PEEK(COMMA));
#endif
	} while (next(COMMA));
#ifdef DEBUG
	if (initdebug)
...
PP(int ssp;)
{
	...
	if (initdebug)
	{
		fprintf(stderr, "type: %d BTYPE %d ISARRAY %d ", type, BTYPE(type), ISARRAY(type));
		fprintf(stderr, "POINTER %d\n", ISPOINTER(type));
		fprintf(stderr, "onetype: %d BTYPE %d ", onetype, BTYPE(onetype));
		fprintf(stderr, "ISARRAY %d POINTER %d\n", ISARRAY(onetype), ISPOINTER(onetype));
	}
#endif
	nbout = 0;
