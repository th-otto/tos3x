...
struct extnode {
	...
	short t_offset;
	short t_reg;
	short t_symbol[SSIZE];		  /* symbol name BUG: should be char */
};
...
struct iob obuf, lbuf, sbuf, ibuf, *obp;
...
extern char const aregtab[];
extern char const dregtab[];

/* Parser flags */
short eflag;					/* IEEE floats */
...
short wflag;					/* don't generate warning mesgs */
#ifndef NOPROFILE
...
int falign PROTO((int type, int flen, int offset));
