...
int _blkio(P(FD *) ccbp, P(long) sector, P(VOIDPTR) buffer, P(long) count, P(int) bdosfunc)
...
#if PCDOS
	register int seccnt;				/* number sectors xferred   */

	if (bdosfunc == B_READ)
		return _pc_readblk(&(ccbp->fcb), sector, buffer, (int) count, SECSIZ);
	else
		return _pc_writeblk(&(ccbp->fcb), sector, buffer, (int) count, SECSIZ);
	UNUSED(seccnt);
#endif
