...
/* is_member of a set */
static int _ismem PROTO((char c, const char *set, int invert));


int _doscan(P(FILE *)sp, P(const char *)fmt, P(char **)aps)
PP(register FILE *sp;)							/* Stream ptr           */
PP(const char *fmt;)								/* Input format spec ptr    */
PP(char **aps;)								/* Argument ptrs ptr        */
{
	...
	register int width;					/* Max field width      */
	...
	while ((c = *fmt++) != 0)
	{
		...
		case '[':						/* input 'one of'       */
			...
			if (noassign)
				...
				db = *aps++;			/* o.w. grab the desired ptr */
