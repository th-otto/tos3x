...
VOID blkmove PROTO((char *to, const char *from, int nn));

#ifdef __OSIF_H__
/* BUG in original library: returns char *, but is not declared */
...
int _blkio PROTO((FD *ccbp, long sector, VOIDPTR buffer, long count, int bdosfunc));
#endif
VOID _pc_chain PROTO((const char *cmdline));
int _ttyinraw PROTO((int flag));
int ttyinraw PROTO((NOTHING));
