...
VOID op_expand PROTO((int op, int type, int ltype));
int skelmatch PROTO((struct tnode *tp, int skinfo));
 ...
 */
int expand(P(struct tnode *) tp, P(int) cookie, P(int) freg, P(struct skeleton *) skp)
...
PP(struct skeleton *skp;)					/* pointer to code skeleton */
{
		...
		case TREE:
			...
			if (flag & S_INDR)
			{
				...
				if (coffset(p))
				{
					...
					{
#ifdef __ALCYON__
						p = tnalloc(INT2L, LONG, 0, 0, p); /* BUG: missing argument */
#else
...
VOID op_expand(P(int) op, P(int) type, P(int) ltype)
PP(int op;)
PP(int type;)
PP(int ltype;)
{
 ...
 */
struct skeleton *match(P(struct tnode *) tp, P(int) cookie, P(int) reg)
 ...
 */
int skelmatch(P(struct tnode *) tp, P(int) skinfo)
PP(struct tnode *tp;)
PP(int skinfo;)
{
