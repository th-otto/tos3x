...
int optim PROTO((struct tnode **tpp));
int harder PROTO((struct tnode *tp, struct tnode *ntp));
VOID addtree PROTO((struct tnode *tp, struct tnode ***clist, struct tnode ***plist));
int ccexpr PROTO((struct tnode **tpp));
int power2 PROTO((struct tnode **tpp));
int multop PROTO((struct tnode **tpp));
int chklong PROTO((struct tnode * tp));
...
struct tnode *fixbfield(P(struct tnode *) tp)						/* returns pointer to fixed tree */
 ...
 */
int optim(P(struct tnode **) tpp)								/* returns pointer to new tree */
PP(struct tnode **tpp;)
{
 ...
 */
struct tnode *commute(P(struct tnode *) tp)						/* returns commuted expression tree */
PP(struct tnode *tp;)
{
 ...
 */
int harder(P(struct tnode *) tp, P(struct tnode *) ntp)							/* returns 1 if tp > ntp, else 0 */
PP(struct tnode *tp;)
PP(struct tnode *ntp;)
{
	short lconst;
 ...
 */
VOID addtree(P(struct tnode *) tp, P(struct tnode ***) clist, P(struct tnode ***) plist)
PP(struct tnode *tp;)						/* pointer to tree */
...
int ccexpr(P(struct tnode **) tpp)								/* returns 1 if changes, 0 otherwise */
...
int chklong(P(struct tnode *) tp)								/* returns 1 if long, 0 otherwise */
PP(struct tnode *tp;)
{
	if (tp->t_op == CLONG)
		return 1;
