 ...
 */
static struct tnode *addptree(P(struct tnode *) tp, P(struct tnode ***) clp)
PP(struct tnode *tp;)
PP(struct tnode ***clp;)
{
	register int op;
...
static int fixresult(P(struct tnode *) tp, P(int) cookie, P(int) reg)
PP(struct tnode *tp;)
...
#ifdef DEBUG
	if (cflag)
		oprintf("fixresult cookie=%d reg=%d op=%d\n", cookie, reg, tp->t_op);
#endif
	switch (cookie)
	{
	case FORCC:
...
static VOID outstrcpy(P(int) lr, P(int) rr, P(int) size)
PP(int lr;)
 ...
 */
static int loadexpr(P(struct tnode *) tp, P(int) cookie, P(int) reg)
...
static int cqmark(P(struct tnode *) tp, P(int) cookie, P(int) reg)					/* returns reg or -1 */
PP(struct tnode *tp;)
PP(int cookie;)
PP(int reg;)
{
...
static int hardrel(P(struct tnode *) tp, P(int) cookie, P(int) reg)				/* returns reg or -1 */
...
#ifdef DEBUG
	if (cflag)
		oprintf("hardrel: cookie=%d op=%d reg=%d\n", cookie, tp->t_op, reg);
#endif
	op = tp->t_op;
 ...
 */
static int ucodegen(P(struct tnode *) tp, P(int) cookie, P(int) reg)				/* returns reg result is in */
PP(struct tnode *tp;)						/* pointer to expression tree */
...
PP(int reg;)								/* first available register */
{
	...
	if ((r = hardrel(tp, cookie, reg)) >= 0)
		return r;
	if (cookie == FORCC && (skp = match(tp, FOREFF, reg)) != 0)
	{
		r = expand(tp, FOREFF, reg, skp);
		if (ISASGOP(tp->t_op) && indexreg(tp->t_left))
			outcmp0(tp->t_left->t_reg, tp->t_left);
	} else if ((skp = match(tp, cookie, reg)) != NULL)
	{
		r = expand(tp, cookie, reg, skp);
...
static int noref(P(struct tnode *) tp, P(int) reg)							/* 4.0 change */
...
static int rcgen(P(struct tnode **) tpp, P(int) cookie, P(int) reg)					/* returns changed flag */
...
#ifdef DEBUG
				if (cflag > 1)
					oprintf("exiting rcgen with ltp type CHAR change %d\n", change);
#endif
				return change;
			}
...
static int rcodegen(P(struct tnode **) tpp, P(int) cookie, P(int) reg)				/* returns changed flag */
PP(struct tnode **tpp;)						/* pointer to tree */
...
static int cdsize(P(struct tnode *) tp)								/* returns data size in bytes */
PP(struct tnode *tp;)
{
	register short type;
...
static short dofarg(P(struct tnode *) tp)								/* returns number of bytes pushed */
PP(struct tnode *tp;)						/* pointer to expression tree */
{
	register short nb;
...
static int dobitadd(P(struct tnode *) tp, P(int) bitno)						/* returns -1 if can't or bitno */
PP(struct tnode *tp;)
PP(int bitno;)
{
...
static int isonebit(P(struct tnode *) tp)							/* returns -1 if not 1 bit, else bitno */
PP(struct tnode *tp;)						/* pointer to tree */
{
	short lconst;
	int32_t bvalue;
...
#ifdef DEBUG
	if (cflag)
		oprintf("codegen op=%d cookie=%d reg=%d\n", tp->t_op, cookie, reg);
#endif
	switch (tp->t_op)
	{
	case CALL:
...
#ifdef DEBUG
	if (cflag)
		oprintf("condbr: dir = %d, lab = %d, reg = %d\n", dir, lab, reg);
#endif
	switch (op)
	{
	case CINT:
