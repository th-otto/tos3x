...
PP(struct libhdr *arptr;)
{
	...
	for (i = 0, lp = arptr->lfname; i < LIBNSIZE; i++, lp++)
		if (putc(*lp, fp) < 0)
			return EOF;
	if (lputl(&arptr->lmodti, fp) < 0)
		return EOF;
	if (putc(arptr->luserid, fp) < 0)
		return EOF;
	if (putc(arptr->lgid, fp) < 0)
		return EOF;
	if (lputw(&arptr->lfimode, fp) < 0)
		return EOF;
	if (lputl(&arptr->lfsize, fp) < 0)
		return EOF;
	if (lputw(&arptr->junk, fp) < 0)
		return EOF;
	return 0;
}
